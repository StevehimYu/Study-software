...
def _sort(word_dict_1):  # 用于开始新一轮的链表
    cover_list = {}
    # word_list = {}
...
def _open():
    ...
    try:
        with open("word.json", "r", encoding="gb2312") as w:
...
def save(dic):
    with open("word.json", "w", encoding="gb2312") as w:
        json.dump(dic, w)
        w.close()
...
def new_test(cover_list, word_dict_1):  # 暂时仅限英语
    """
    cover_list 结构：
    cover_list:{
        "number": xx,
        ...
        if ifDaoRu == 1:
            ...
            while True:
                ...
                if op == 1:
                    ...
                    new_test(cover_list, word_dict_1)
                elif op == 8:
